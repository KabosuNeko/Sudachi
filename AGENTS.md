...
- **HLS ad-stripping** at `play_video()`: `.m3u8` URLs pass through `hls_fetch_clean` before playback — mid-roll ad segments (patterns in the `HLS_AD_PATTERNS` awk ERE variable, extendable) are dropped from the media playlist; the cleaned playlist is cached as `<hash>-clean.m3u8` in `cache/`; any fetch/filter failure falls back to the raw URL (playback never breaks). mpv gets `--cache=yes` for HLS (never `--force-seekable=yes` — breaks HLS, mpv#11990); for the local cleaned playlist mpv additionally forces `--demuxer-lavf-format=hls` + a widened `protocol_whitelist` (ffmpeg refuses https segments with the default file,crypto,data list — stalls every few seconds) + `--demuxer-lavf-linearize-timestamps=yes` (PTS jumps where ad segments were removed — without it seek across those points resets to the start / audio desyncs). Downloads keep the raw stream.
